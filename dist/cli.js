...
import { existsSync as existsSync15, writeFileSync as writeFileSync13 } from "fs";
...
function ensureRootGitignoreEntry(rootDir, loopsDir) {
  ...
  const entries = [ignore, unignore];
  if (!existsSync2(path)) {
    const header = `# Auto-added by @ekolabs/claude-design-loop. Local design-loop
...
function hasIgnoreEntry(content, loopsDir) {
  const trimmedDir = loopsDir.replace(/\/+$/, "");
  ...
  for (const raw of content.split("\n")) {
    const line2 = raw.replace(/^\s+|\s+$/g, "");
    if (!line2 || line2.startsWith("#") || line2.startsWith("!")) continue;
    if (candidates.has(line2)) return true;
  }
  return false;
}
...
var svelteAdapter = {
  ...
  async discoverRoutes(opts) {
    const root = resolve3(opts.routesDir);
    ...
    function visit(dir) {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        const rel = relative(root, dirname2(full));
  ...
  async apply(ctx) {
    ...
    for (const htmlPath of htmlPages) {
      ...
      for (const cssHref of linkedCss) {
        ...
        if (existsSync3(cssPath) && statSync(cssPath).isFile()) {
...
${readFileSync2(cssPath, "utf8")}`
      ...
      if (bodyIsEmpty) {
        ...
        mkdirSync2(sourcesDir, { recursive: true });
      ...
      writeFileSync2(outPath, svelteSource, "utf8");
...
import { copyFileSync as copyFileSync2, existsSync as existsSync4, mkdirSync as mkdirSync3, readdirSync as readdirSync2 } from "fs";
...
import { basename as basename3, dirname as dirname3, join as join3, relative as relative2, resolve as resolve4 } from "path";
...
var nextjsAdapter = {
  name: "nextjs",
  async discoverRoutes(opts) {
    const root = resolve4(opts.routesDir);
  ...
  async apply(ctx) {
    ...
    for (const htmlPath of htmlPages) {
      ...
      for (const cssHref of linkedCss) {
        ...
        if (existsSync5(cssPath) && statSync2(cssPath).isFile()) {
          inlinedCssBlocks.push(
            `/* From bundle/${cssHref} */
${readFileSync3(cssPath, "utf8")}`
          );
        }
      }
      const allStyles = [styles, ...inlinedCssBlocks].filter(Boolean).join("\n\n");
      ...
      if (allStyles) {
        writeFileSync3(cssModulePath, allStyles + "\n", "utf8");
      ...
      writeFileSync3(outPath, tsxSource, "utf8");
      translatedFiles.push(outPath);
    }
    return {
      translatedFiles,
...
import { existsSync as existsSync6, readdirSync as readdirSync4, statSync as statSync3, writeFileSync as writeFileSync5 } from "fs";
import { basename as basename4, relative as relative3, resolve as resolve6 } from "path";
...
function loopPaths(loopsRoot, id) {
  const root = resolve5(loopsRoot, id);
  return {
    id,
    root,
    inputsDir: join4(root, "inputs"),
    bundleDir: join4(root, "bundle"),
    outputDir: join4(root, "output"),
    briefPath: join4(root, "brief.md"),
    manifestPath: join4(root, "manifest.json"),
    reviewChecklistPath: join4(root, "review-checklist.md"),
    verifyReportPath: join4(root, "verify-report.md")
  };
}
function ensureLoopDirs(paths) {
  ensureLoopsRootGitignore(dirname4(paths.root));
  mkdirSync5(paths.root, { recursive: true });
  mkdirSync5(paths.inputsDir, { recursive: true });
}
function writeManifest(paths, manifest) {
  writeFileSync4(paths.manifestPath, JSON.stringify(manifest, null, 2) + "\n", "utf8");
}
function readManifest(paths) {
  return JSON.parse(readFileSync4(paths.manifestPath, "utf8"));
}
...
function readLine() {
  ...
  return new Promise((resolve17) => lineWaiters.push(resolve17));
}
...
async function promptChoiceTTY(question, choices, defaultKey) {
  return new Promise((resolve17, reject) => {
    ...
    try {
      ...
      promptChoiceLineBased(question, choices, defaultKey).then(resolve17, reject);
      return;
    }
    stdin.resume();
...
`
    ...
    function onData(chunk) {
      ...
      if (ch === "\r" || ch === "\n") {
        cleanup();
        resolve17(items[selected].value);
        return;
      }
...
async function writeCursorPrompt(args) {
  const loopsRoot = resolve6(args.rootDir, args.config.loopsDir ?? "design-loops");
  ...
  const promptPath = resolve6(paths.outputDir, "CURSOR_PROMPT.md");
  writeFileSync5(promptPath, prompt, "utf8");
...
function describeScaffold(absScaffold, rootDir) {
  ...
  const sourcesAbs = resolve6(absScaffold, "..", "sources", componentName);
  let sourcesRel = null;
  let sourceFiles = [];
  if (existsSync6(sourcesAbs) && statSync3(sourcesAbs).isDirectory()) {
    sourcesRel = relative3(rootDir, sourcesAbs);
    sourceFiles = readdirSync4(sourcesAbs).filter((n) => !n.startsWith(".")).sort();
  }
...
async function runApply(args) {
  const loopsRoot = resolve7(args.rootDir, args.config.loopsDir ?? "design-loops");
  const paths = loopPaths(loopsRoot, args.loopId);
  if (!existsSync7(paths.bundleDir)) {
    throw new Error(
      `Bundle directory missing for ${args.loopId}. Run \`design-loop pull\` first.`
    );
  }
  mkdirSync6(paths.outputDir, { recursive: true });
  ...
  const summaryPath = resolve7(paths.outputDir, "APPLY_SUMMARY.md");
  writeFileSync6(summaryPath, summary, "utf8");
...
function parseChecklist(path) {
  ...
  const md = readFileSync5(path, "utf8");
...
async function captureWithContext(context, opts) {
  ...
  for (const width of opts.breakpoints) {
    ...
    const screenshotPath = join5(opts.outDir, `screenshot-${width}.png`);
  ...
  const domSnapshotPath = join5(opts.outDir, "dom.yaml");
  writeFileSync7(domSnapshotPath, domSnapshot, "utf8");
  const classFrequencyPath = join5(opts.outDir, "class-frequency.json");
  writeFileSync7(
    classFrequencyPath,
...
async function runBrief(args) {
  ...
  const paths = loopPaths(resolve8(args.rootDir, config.loopsDir), id);
  ...
  writeFileSync8(paths.briefPath, briefMarkdown, "utf8");
  const manifest = {
    id,
...
function defaultAuthPaths() {
  const root = join6(homedir(), ".config", "design-loop");
  return {
    storageState: join6(root, "auth.json"),
    profileDir: join6(root, "chromium-profile")
  };
}
...
async function dumpFailureState(page, label) {
  try {
    const root = join6(homedir(), ".config", "design-loop", "failures");
    mkdirSync7(root, { recursive: true });
    const stamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
    const slug = label.replace(/[^a-z0-9-]+/gi, "_").slice(0, 40);
    const base = join6(root, `${stamp}-${slug}`);
    const aria = await page.locator("body").ariaSnapshot().catch(() => "<aria snapshot failed>");
    writeFileSync9(`${base}.aria.yaml`, aria, "utf8");
    writeFileSync9(`${base}.url.txt`, page.url(), "utf8");
...
async function loginInteractive(authPaths) {
  ...
  mkdirSync7(authPaths.profileDir, { recursive: true });
...
async function submitToClaudeDesign(opts) {
  ...
  mkdirSync7(opts.authPaths.profileDir, { recursive: true });
...
function lockPath(loopsRoot) {
  return resolve9(loopsRoot, LOCK_FILE);
}
function checkLock(loopsRoot) {
  const path = lockPath(loopsRoot);
  if (!existsSync8(path)) return { active: false, alive: false };
  let info;
  try {
    info = JSON.parse(readFileSync6(path, "utf8"));
  } catch {
...
function acquireLock(loopsRoot, opts) {
  ...
  writeFileSync10(path, JSON.stringify(info, null, 2) + "\n", "utf8");
  let released = false;
  const release = () => {
    if (released) return;
    released = true;
...
import { extname, join as join7, resolve as resolve10 } from "path";
async function runPull(args) {
  const loopsRoot = resolve10(args.rootDir, args.config.loopsDir ?? "design-loops");
  const paths = loopPaths(loopsRoot, args.loopId);
  ...
  mkdirSync8(paths.bundleDir, { recursive: true });
  ...
  const recDestPath = join7(paths.root, "RECOMMENDATIONS.md");
  ...
  const checklistPath = writeReviewChecklist(
    ...
    recPath ? readFileSync7(recPath, "utf8") : null,
    args.loopId
  );
  const manifest = readManifest(paths);
  manifest.bundle = {
...
async function materializeBundle(source, destDir) {
  if (/^https?:\/\//i.test(source)) {
    const tmp = join7(destDir, ".bundle.archive");
...
function surfaceRecommendations(bundleDir, destPath) {
  ...
  const content = readFileSync7(found, "utf8");
  writeFileSync11(destPath, content, "utf8");
  return destPath;
}
...
function writeReviewChecklist(destPath, recommendationsMd, loopId) {
  ...
  writeFileSync11(destPath, body, "utf8");
  return destPath;
}
...
async function runFetch(args) {
  const config = withDefaults(args.config);
  const loopsRoot = resolve11(args.rootDir, config.loopsDir);
  const paths = loopPaths(loopsRoot, args.loopId);
  if (!existsSync10(paths.manifestPath)) {
...
async function runFetchInner(args, config, paths) {
  ...
  if (!existsSync10(authPaths.storageState)) {
    throw new Error(
...
async function runResume(args) {
  ...
  const paths = loopPaths(loopsRoot, args.loopId);
  if (!existsSync11(paths.manifestPath)) {
    throw new Error(`Loop ${args.loopId} not found at ${paths.root}.`);
  }
  const lock = acquireLock(loopsRoot, {
    command: "resume",
...
async function runSubmitInner(args, config, paths) {
  if (!existsSync12(paths.briefPath)) {
...
function collectAttachments(inputsDir) {
  if (!existsSync12(inputsDir)) return [];
  return readdirSync6(inputsDir).filter((name) => /\.(png|jpe?g|webp|gif|pdf)$/i.test(name)).map((name) => join8(inputsDir, name));
}

// src/lib/verify.ts
import { existsSync as existsSync13, mkdirSync as mkdirSync9, readFileSync as readFileSync8, writeFileSync as writeFileSync12 } from "fs";
import { join as join9, relative as relative4, resolve as resolve14 } from "path";
async function runVerify(args) {
  const config = withDefaults(args.config);
  const loopsRoot = resolve14(args.rootDir, config.loopsDir);
  const paths = loopPaths(loopsRoot, args.loopId);
  if (!existsSync13(paths.root)) {
    throw new Error(`Loop ${args.loopId} not found.`);
  }
  const manifest = readManifest(paths);
  const afterDir = join9(paths.outputDir, "after");
  mkdirSync9(afterDir, { recursive: true });
  ...
  const beforeRefs = manifest.breakpoints.map((bp) => ({
    width: bp,
    path: relative4(paths.root, join9(paths.inputsDir, `screenshot-${bp}.png`))
  }));
  ...
  writeFileSync12(paths.verifyReportPath, md, "utf8");
...
// src/cli.ts
{
  ...
  process.emit = function patchedEmit(event, value, ...rest) {
    if (event === "warning" && value && typeof value === "object" && "code" in value && value.code === "MODULE_TYPELESS_PACKAGE_JSON") {
      return false;
    }
...
program.command("brief").description("Capture a route and write a short design brief").argument("<route>", "route path, e.g. / or /canonical").option(
  "--breakpoints <list>",
  ...
  const { config, rootDir } = await loadConfig();
  const breakpoints = opts.breakpoints ? opts.breakpoints.split(",").map((s) => Number(s.trim())).filter(Boolean) : void 0;
...
program.command("submit").description(
...
).argument("<loopId>", "loop id (the directory name under design-loops/)").option("--headed", "show the browser window (recommended for review)", true).option("--no-headed", "run the browser headless (CI / quick smoke tests)").option(
  "--fidelity <fidelity>",
  "wireframe | high-fidelity (default: high-fidelity)"
).option("--project-name <name>", "override the new project name (default: loop id)").option(
  "--no-interactive",
  ...
  async (loopId, opts) => {
    const { config, rootDir } = await loadConfig();
    const fidelity = opts.fidelity ? assertFidelity(opts.fidelity) : void 0;
    await runSubmit({
      config,
      rootDir,
      loopId,
...
).argument("<loopId>", "loop id").option(
  "--project-url <url>",
  "override the manifest-saved project URL (claude.ai/design/p/...)"
).option("--headed", "show the browser window (recommended)", true).option("--no-headed", "run the browser headless (CI / quick smoke tests)").option(
  "--no-interactive",
...
program.command("fetch").description(
  ...
  "--project-url <url>",
  "override the manifest-saved project URL (claude.ai/design/p/...)"
).option("--headed", "show the browser window", true).option("--no-headed", "run the browser headless (CI / quick smoke tests)").option(
  "--no-pull",
...
program.command("pull").description("Expand a Claude Design handoff bundle into a loop").argument("<loopId>", "loop id").option("--bundle-url <url>", "handoff bundle URL (api.anthropic.com/v1/design/h/...)").option("--bundle-path <path>", "path to a downloaded bundle .zip").action(
  async (loopId, opts) => {
...
program.command("apply").description("Translate the bundle into framework-native scaffolds").argument("<loopId>", "loop id").option(
  "--no-interactive",
  'skip the "copy prompt to clipboard?" question (CI / scripts)'
).action(async (loopId, opts) => {
  const { config, rootDir } = await loadConfig();
  await runApply({
    config,
    rootDir,
...
program.command("verify").description("Re-capture the route after apply and write a comparison report").argument("<loopId>", "loop id").action(async (loopId) => {
  const { config, rootDir } = await loadConfig();
  await runVerify({ config, rootDir, loopId });
});
program.command("login").description("One-time interactive login to claude.ai/design").action(async () => {
  const authPaths = defaultAuthPaths();
  await loginInteractive(authPaths);
});
program.command("systems").description(
...
).option("--headed", "show the browser window (default true; --no-headed to hide)", true).option("--no-headed", "run the browser headless").action(async (opts) => {
  const { config } = await loadConfig();
  const authPaths = defaultAuthPaths();
  section("Discovering design systems on claude.ai/design");
  const systems = await listDesignSystems({ authPaths, headed: opts.headed !== false });
  if (!systems.length) {
    warn("No design systems found. Are you logged in? Try `design-loop login`.");
    closePromptIO();
    return;
  }
  const configured = getDesignSystems(config);
  const configuredByName = new Map(
    configured.map((s) => [s.name.toLowerCase(), s])
  );
  const discoveredByName = new Map(
    systems.map((s) => [s.name.toLowerCase(), s])
  );
  section("On claude.ai/design");
  for (const s of systems) {
    const inConfig = configuredByName.get(s.name.toLowerCase());
    const status = !inConfig ? colors.dim("not in your config") : inConfig.id === s.id ? colors.green("matches config") : inConfig.id ? colors.yellow(`config has different id: ${inConfig.id}`) : colors.yellow("config entry missing id \u2014 paste this");
    console.log(`  ${colors.bold(s.name)}`);
    console.log(`    id:     ${colors.cyan(s.id)}`);
  ...
  if (orphaned.length) {
    section("In your config but NOT on claude.ai/design");
    for (const o of orphaned) {
      warn(`"${o.name}"${o.id ? ` (id: ${o.id})` : ""}`);
      hint(
...
).argument("<route>", "route path, e.g. / or /canonical").option("--breakpoints <list>", "comma-separated viewport widths in px").option("--intent <text>", "optional one-line intent for this round").option("--headed", "show the browser window", true).option("--no-headed", "run the browser headless (CI / quick smoke tests)").option("--fidelity <fidelity>", "wireframe | high-fidelity (default: high-fidelity)").option(
  "--no-interactive",
...
program.command("init").description("Bootstrap a .design-loop.config.ts in the current directory").action(async () => {
  const dest = resolve16(process.cwd(), ".design-loop.config.ts");
  if (existsSync15(dest)) {
    console.log(`Config already exists at ${dest}. Skipping.`);
    return;
  }
  const stub2 = `import { defineConfig } from '@ekolabs/claude-design-loop';
...
`;
  writeFileSync13(dest, stub2, "utf8");
  console.log(`Wrote ${dest}.`);
  const gi = ensureRootGitignoreEntry(process.cwd(), "design-loops");
  const rule2 = `${gi.entries[0]} (with \`${gi.entries[1]}\`)`;
  if (gi.action === "created") {
    console.log(`Created ${gi.path} with \`${rule2}\`.`);
  } else if (gi.action === "appended") {
    console.log(`Added \`${rule2}\` to ${gi.path}.`);
  }
  console.log(`Next: \`design-loop login\`, then \`design-loop\` for the wizard.`);
});
function assertFidelity(value) {
